...
export default function Home() {
  ...
  function onFile(e) {
    ...
    reader.onload = () => {
      ...
      setImgData({ base64, mediaType: file.type });
    };
  ...
  async function generate() {
    ...
    setLoading("우리 어린이집만의 소개글을 쓰고 있어요…");
    setError("");
    try {
  ...
  function restart() {
    ...
    setForm(EMPTY); setResult(null); setError("");
  }
  ...
  return (
    <div className="wrap">
      <div className="header">
        <h1>🌱 어린이집 브랜딩 구축하기</h1>
        <p>우리 어린이집의 마음이 담긴 인스타그램 프로필 소개글을 만들어 드려요.</p>
      </div>
      ...
      {loading && (
        <div className="card"><div className="loading"><div className="spinner" />{loading}</div></div>
      )}
      ...
      {!loading && step === "questions" && (
        <>
          ...
          <div className="card">
            ...
            <div className="field">
              <label>아이·부모에게 전하고 싶은 마음 한마디 <span className="hint">마음을 울리는 문장</span></label>
              <textarea value={form.heartMessage} onChange={(e) => set("heartMessage", e.target.value)} placeholder="예) 오늘 하루도 아이답게, 마음껏 자라나기를 바랍니다." />
            </div>

            <div className="field">
              <label>우리 어린이집의 자랑거리·특징 <span className="hint">해당되는 것을 눌러 주세요</span></label>
              <div className="chips">
            ...
            <div className="btn-row">
              <button className="btn gray" onClick={restart}>처음으로</button>
            </div>
            {error && <div className="err">{error}</div>}
          </div>
        </>
      ...
      {!loading && step === "result" && result && (
        <>
          <div className="card">
            ...
            {result.tip && (
              <div className="tag-good" style={{ marginTop: 6 }}>💡 브랜딩 팁: {result.tip}</div>
            )}
          </div>
          ...
          <div className="card">
            ...
            <div className="btn-row">
              <button className="btn gray" onClick={restart}>처음부터 새로</button>
            </div>
          </div>
        </>
      )}
...
function ResultBlock({ title, text, showCount }) {
